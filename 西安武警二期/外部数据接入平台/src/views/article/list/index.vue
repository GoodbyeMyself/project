<!-- 数据源管理页面 -->
<template>
  <!-- Auto Comment: Component Summary: This component renders UI for '外部数据接入平台/src/views/article/list/index.vue'. -->
...
<script setup lang="ts">
  // Auto Comment: Component Script Notes: This script block manages state, events, and data flow for '外部数据接入平台/src/views/article/list/index.vue'.
  // Auto Comment: Maintenance Hint: Keep business rules explicit and avoid implicit side effects between handlers.
  defineOptions({ name: 'ArticleList' })

  // Logic Note: Reactive state 'keyword' stores mutable runtime data used by this component.
  const keyword = ref('')
  // Logic Note: Reactive state 'status' stores mutable runtime data used by this component.
  const status = ref('全部状态')

  // Logic Note: Reactive state 'dataSourceList' stores mutable runtime data used by this component.
